"""Phase 12 analysis --- DGN-style multi-head attention robustness check.

Reads a results dir that already contains qmix / mlp_qmix / gnn_qmix /
gat_qmix runs plus the newly-added dgn_qmix runs, and reports the DGN
contrasts under the SAME protocol as Phase 8/9/10 (final-window mean;
Welch + Holm over the DGN family + Mann-Whitney; Cohen's d):

    graph (multi-head): DGN-QMIX - MLP-QMIX  (does multi-head relational
                                              attention beat no-graph?)
    vs single-head GAT: DGN-QMIX - GAT-QMIX
    vs fixed GCN      : DGN-QMIX - GNN-QMIX
    net               : DGN-QMIX - QMIX

Headline question is the first contrast: if the multi-head dot-product
attention used by DGN/G2ANet also fails to beat MLP-QMIX, the graph penalty
is not an artefact of a weak GNN. DGN-QMIX has by far the most parameters,
so DGN < MLP is the most conservative form of the claim.

Usage:
  python scripts/phase12_dgn_analysis.py --results results/phase8 --tag coordgrid_ring
  python scripts/phase12_dgn_analysis.py --results results/phase9_mpe --tag mpe_spread
"""
...
def main() -> int:
    ...
    ap.add_argument("--tag", type=str, default="dgn")
    ...
    if "dgn_qmix" not in set(df["algo"]):
        print(f"[phase12] no dgn_qmix runs in {args.results} yet", file=sys.stderr)
        return 1
    print(f"[phase12:{args.tag}] loaded {len(df)} runs, "
    ...
    contrasts = [
        ("graph (multi-head): DGN-QMIX - MLP-QMIX", "dgn_qmix", "mlp_qmix"),
        ("vs single-head GAT: DGN-QMIX - GAT-QMIX", "dgn_qmix", "gat_qmix"),
        ("vs fixed GCN: DGN-QMIX - GNN-QMIX", "dgn_qmix", "gnn_qmix"),
        ("net: DGN-QMIX - QMIX", "dgn_qmix", "qmix"),
    ]
    out_rows = []
    print(f"\n=== DGN contrasts --- {args.tag} ===")
    for n in sorted(df["n_agents"].unique()):
        ...
        for label, a, b in contrasts:
            ...
            recs.append([n, label, delta, cohend(xa, xb), raw_p, mwu_p])
            raw_ps.append(raw_p)
        if not recs:
            continue
        ...
        for rec, h in zip(recs, holm):
            ...
            print(f"  N={n_}  {label:<42} delta={delta:+8.2f}  d={d:+5.2f}  "
                  f"holm={h:.3f}  mwu={mwu_p:.3f}  {flags}")

    if out_rows:
        outp = args.results / f"dgn_verdict__{args.tag}.csv"
        pd.DataFrame(out_rows).to_csv(outp, index=False)
        print(f"\n[phase12:{args.tag}] -> {outp}")
    return 0
