...
Q_tot = sum_i Q_i. Centralised training with the sum mixer; decentralised
execution because each agent only needs its own Q_i to act.
"""
...
from .networks import QNet, VDNMixer
...
class VDN(BaseAgent):
    ...
    def __init__(self, cfg: AlgoConfig, device: torch.device):
        ...
        self.mixer_target = VDNMixer().to(device)  # parameter-free, but kept for symmetry
    ...
    def learn(self, batch, adj: np.ndarray) -> dict:  # noqa: ARG002
        ...
        q_tot = self.mixer(q_taken, state).squeeze(-1)  # (B,)

        with torch.no_grad():
            q_next_all = self.q_target(next_obs)
            # double-DQN-ish: select max with online net, evaluate with target.
            # For pure VDN we just use target max.
