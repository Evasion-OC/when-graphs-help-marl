...
CELLS = [
    ...
    code(
        ...
        "\n",
        "# The repo is private; we need a token to clone too (not just to push).\n",
        "# We look in Colab Secrets first (same names accepted as cell 10),\n",
        "# then fall back to an interactive prompt. The token is stripped from\n",
        "# the saved git config after clone so it doesn't sit on disk.\n",
        "def _resolve_token():\n",
        "    try:\n",
        "        from google.colab import userdata\n",
        "        for name in ('GNNMARL_GITHUB_TOKEN', 'GITHUB_TOKEN'):\n",
        "            try:\n",
        "                v = (userdata.get(name) or '').strip()\n",
        "                if v:\n",
        "                    print(f'using token from Colab Secret: {name}')\n",
        "                    return v\n",
        "            except Exception:\n",
        "                continue\n",
        "    except Exception:\n",
        "        pass\n",
        "    import getpass\n",
        "    return getpass.getpass('GitHub PAT (repo scope; input hidden): ').strip()\n",
        "\n",
        "if not os.path.exists('when-graphs-help-marl'):\n",
        "    _tok = _resolve_token()\n",
        "    if not _tok:\n",
        "        raise RuntimeError('No GitHub token available; cannot clone private repo.')\n",
        "    _auth_url = REPO_URL.replace('https://', f'https://x-access-token:{_tok}@')\n",
        "    subprocess.check_call(['git', 'clone', '--branch', BRANCH, '--depth', '1', _auth_url, 'when-graphs-help-marl'])\n",
        "    # Wipe the token from the local remote config now that the clone is done.\n",
        "    subprocess.check_call(['git', '-C', 'when-graphs-help-marl', 'remote', 'set-url', 'origin', REPO_URL])\n",
        "    del _tok, _auth_url\n",
    ...
    code(
        "# Phase 8 is the confound-killer + capacity control (the revision experiment).\n",
        "# It is ON by default; the earlier phases are already in the repo.\n",
        "RUN_PHASE_1 = False   # pilot: 4 algos x 3 seeds x 50k steps  (~30-90 min)\n",
        "RUN_PHASE_2 = False   # E1 sweep: 4 algos x 2 N x 2 graphs x 3 seeds x 30k\n",
        "RUN_PHASE_4 = False   # depth x diameter ablation\n",
        "RUN_PHASE_8 = True    # confound-killer: long-horizon + MLP control\n",
        "\n",
        "# Phase 8 knobs. Defaults: 5 algos x 2 N x 5 seeds x 150k steps = 50 runs.\n",
        "# On a Colab T4 this is roughly 6-9 h. Trim seeds or set PHASE8_NS='8' to\n",
        "# fit a shorter session; bump PHASE8_STEPS to 200000 for an even stronger\n",
        "# anti-undertraining argument.\n",
        "PHASE8_STEPS = 150000\n",
        "PHASE8_SEEDS = 5\n",
        "PHASE8_ALGOS = 'qmix,mlp_qmix,gnn_qmix,vdn,iql'\n",
        "PHASE8_NS = '4,8'\n",
    ),
    md(
        "## 5. Phase 1 --- pilot",
    ...
    md(
        "## 7b. Phase 8 --- confound-killer + capacity control",
        "",
        "Trains every algorithm for `PHASE8_STEPS` env steps (default 150k, vs "
        "Phase 2's 30k) with `PHASE8_SEEDS` seeds (default 5, vs 3) on the decisive "
        "`ring` cells, and adds **MLP-QMIX** --- the parameter-matched no-graph "
        "control (identical to GNN-QMIX at 24,518 params, GCN encoder replaced by a "
        "shape-identical MLP). Answers the two biggest reviewer objections at once: "
        "(A) does QMIX > GNN-QMIX survive long training (undertraining confound), and "
        "(B) is any GNN-QMIX gap the graph or just capacity (GNN-QMIX vs MLP-QMIX at "
        "matched params).",
    ),
    code(
        "if RUN_PHASE_8:\n",
        "    !PYTHONIOENCODING=utf-8 python scripts/phase8_confound_sweep.py \\\n",
        "        --steps {PHASE8_STEPS} --seeds {PHASE8_SEEDS} \\\n",
        "        --algos {PHASE8_ALGOS} --ns {PHASE8_NS} \\\n",
        "        2>&1 | tee results/phase8_log.txt\n",
        "    !PYTHONIOENCODING=utf-8 python scripts/phase8_analysis.py 2>&1 | tail -40\n",
        "    checkpoint('after_phase8')\n",
        "else:\n",
        "    print('skipped phase 8')\n",
    ),
    md(
        "## 8. Build the paper",
        "",
