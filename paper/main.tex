...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item \textbf{The positive endpoint: when an agent needs private
        ...
        (stabilisation-matched learned attention also floors over all-to-all
        within budget, while an exploratory multi-head arm gains $+5.4$
...
strengthens. At 150k the complete-graph attention arms remain pinned at the
floor (\texttt{gat\_complete} $\SCkGatComplete$, \texttt{dgn\_complete}
$\SCkDgnComplete$ vs.\ the $\SCkMlp$ no-communication floor), while \gtrue\
itself keeps learning (\SCtrueN$\,\to\,$\SCkTrue), so the gap \emph{grows} from
$d{\approx}\SCtrueAttCompD$ at 30k to $d{\approx}\SCkTrueAttCompD$ at 150k
(\gtrue\ over \texttt{gat\_complete}/\texttt{dgn\_complete} by
$\SCkTrueGatCompGap$/$\SCkTrueDgnCompGap$, $p_{\text{Holm}}<10^{-4}$, complete
seed separation). Crucially the \emph{same} attention architectures train well
...
never leave the floor while every true-graph arm climbs monotonically, so
...
\begin{table}[t]
  \centering
  \caption{\textbf{Attention over all-to-all never leaves the floor---at the
  ...
  multi-head DGN---climbs monotonically; the \emph{same} attention architectures
  over the \emph{complete} graph stay pinned at the no-communication floor
  ($\SCkMlp$) for the entire budget. The floor is therefore structural, not
  under-training.}
  \label{tab:attn150k}
  \small
