...
% =========================================================================
\begin{abstract}
We locate both endpoints of a boundary for \emph{when} graph structure helps
cooperative multi-agent reinforcement learning (MARL): a controlled study on
\coordgrid, a cooperative gridworld whose coordination graph is itself the
experimental variable, with the encoder, mixer, optimiser, and parameter
count held fixed across algorithms. The benefit of a graph is real but
conditional, and its source is the graph's \emph{structure}, not the channel
or the added capacity. When an agent must act on private information held by
one specific, unobserved other agent, a graph whose edges match the task's
coordination structure decisively outperforms \emph{all-to-all}
communication---which carries the partner's signal but must select it---a
density-matched wrong graph, and a parameter-matched no-graph control, at
byte-identical parameters and observations (Cohen's $d \approx 5$, complete
seed-level separation over 12 seeds, Holm-corrected). Even learned attention
recovers the partner's signal only on the right graph---and only there even when
given $5\times$ the training budget, at which point the gap widens
($d{\approx}5\!\to\!12$) rather than closing. The effect holds
across team sizes and observability, and replicates out of harness in a
non-spatial referential game, where the matched graph near-solves the task
(${\approx}96\%$ of maximum) while every rival stays far below. When
coordination is instead incidental or convention-reducible, the same
architecture confers \emph{no benefit} over the parameter-matched baseline,
which the control attributes to the graph aggregation rather than capacity;
the commonly used \emph{unstabilised} operator is moreover actively harmful
...
this is an optimisation pathology that stabilisation removes, recovering the
graph to mere parity. The resulting rule: route along task-relevant edges,
not all-to-all, only when agents must act on information held by specific
others they cannot observe \emph{and} the coordination topology is known or
reliably estimable. These are controlled, constructed tasks; replication on
standard benchmarks with private per-agent goals is future work. Code,
configurations, and data accompany the submission.
\end{abstract}
...
This reading also reconciles a real tension in the literature:
\algoname{DGN} and \algoname{G2ANet} report their gains in
\emph{partially observable} settings---the regime our positive endpoint
isolates---while our controlled negatives arise in full-information regimes,
where published graph methods are rarely tested against capacity-matched
controls. On this reading the literature's positive results and our negative
ones are two sides of the same boundary, and the discriminator is the task's
information structure, not the algorithm.
...
not themselves pre-registered. All confirmatory cells use $\ge 12$ seeds and
...
rules out the ``the complete-graph arm never optimised'' objection. Crucially the
...
task's coordination edges}. Crucially, this is the \emph{same} stabilised
