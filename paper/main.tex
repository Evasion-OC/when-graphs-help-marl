...
graph-diameter grid (H3). Two findings emerge. First, on the pilot
ring graph at $N=4$, \gnnqmix\ \emph{underperforms} structurally
simpler \qmix\ in both mean return and seed-level stability ---
opposite to the prevailing claim. Second, the depth--diameter
...
catastrophic across every diameter tested. The practitioner rule we
recommend is correspondingly blunter than H3 anticipated:
...
\paragraph{Where graph structure helps.} Across the conditions tested in
Sections~\ref{sec:results:scale}--\ref{sec:results:ablation}, the
\gnnqmix\ encoder helps over the structurally identical \qmix\ baseline
\emph{only} when [PLACEHOLDER: insert the H1/H2 condition that held]. In
the remaining regimes the mixer-only baseline is competitive or better.
This is a more constrained reading of graph-based MARL than the prevailing
positive framing in the literature \citep{jiang_2020_dgn,liu_2020_g2anet,
...
\emph{First}, on the simplest pilot condition (ring graph, $N=4$, 50k
env steps), \gnnqmix\ does not beat \qmix\ --- it is markedly worse in
mean and dominated in stability. The prevailing positive picture of
graph-based MARL does not survive even a sanity-check comparison in
the regime that should be most favourable to it.
