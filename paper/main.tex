...
% =========================================================================
\begin{abstract}
Graph neural networks are widely used to add coordination structure to
cooperative multi-agent reinforcement learning (MARL), but gains are
rarely separated from the capacity they add. We contribute a
controlled-attribution method that does. On \coordgrid, a cooperative gridworld
...
density-matched \emph{wrong}-graph control (the right edges vs.\ merely more
edges). This reconciles a tension: graph-MARL methods such as
\algoname{DGN} and \algoname{G2ANet} report gains in partially observable
settings, rarely against capacity-matched controls; the
discriminator between those positives and full-information negatives is the
...
graph, even given $5\times$ the training budget, where the gap
widens ($d{\approx}5\!\to\!12$) rather than closes. The advantage holds across
team sizes and observability, and replicates out of harness in a non-spatial
referential game (matched graph ${\approx}96\%$ of maximum, rivals far below).
When coordination is instead incidental or convention-reducible, the same
...
stabilisation removes this optimisation pathology, recovering the graph to
parity. The rule: route along task-relevant edges, not all-to-all---and only
when the coordination topology is known or reliably estimable. These are
controlled, constructed tasks; replication on standard benchmarks with private
per-agent goals is future work. Code, configurations, and data accompany the
submission.
\end{abstract}
...
\emph{convention-reducible}, by which we mean solvable by a fixed shared
convention the agents can adopt without exchanging private state (e.g.\
...
parameter-matched no-graph control. To our knowledge, no published work---through
2026---isolates the contribution of graph \emph{structure} as we do: fixing the
topology and contrasting a task-matched graph against a density-matched wrong
graph and all-to-all at byte-identical parameters and observations. That is the
...
150k) rather than asserting impossibility in principle. The result stands: an
