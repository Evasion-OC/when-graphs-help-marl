...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item We implement \iql, \vdn, \qmix, and \gnnqmix\ in a single harness
        ...
        the GNN. To address Gap~2 directly, we add \mlpqmix: a
...
coordination graph is itself the experimental variable, plus a
parameter-matched no-graph control. Three empirical findings carry the
paper.

\emph{First}, \gnnqmix\ does not beat \qmix\ --- it is markedly worse in
mean and dominated in stability --- and the capacity control
\emph{identifies} the cause. The no-graph \mlpqmix, with byte-identical
parameter count, matches \qmix, while \gnnqmix\ falls well below both at
$N\in\{4,8\}$ ($d=\PEgraphDNeight$ at $N=8$; significant under
Mann--Whitney and Holm-corrected Welch). So the penalty is the graph
aggregation itself, not the added capacity; it survives $5\times$ the
training budget, ruling out undertraining; and it grows with team size.
The prevailing positive picture of graph-based MARL does not survive a
controlled comparison in the regime that should be most favourable to it.

\emph{Second}, the pre-registered receptive-field-alignment heuristic
($L \approx d$) is falsified in this setting. The data show instead a
...
\emph{Third}, the graph does not merely fail to help on average --- it
destabilises training, inflating the across-seed standard deviation at
$N=8$ by roughly $9\times$ relative to the parameter-matched no-graph
control. Reliability, not just mean return, is a casualty of the
relational bias here.

The broader conclusion is that ``does graph structure help in MARL?''
is a question that requires controlled measurement --- including a
capacity-matched control --- to answer, and that on the task most
favourable to a graph-aware model the answer, at the scales we can
test, is ``no, and it actively hurts.'' We hope the released harness
and the \mlpqmix\ control make those measurements cheaper and cleaner
to run for future architectures and tasks.
