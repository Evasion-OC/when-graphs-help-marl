...
% Status: All five findings measured; ready for submission review.
%
% Build (default: anonymized submission version):
%   tectonic --untrusted main.tex
%
% For a non-anonymized preprint, change \usepackage{tmlr} to
% \usepackage[preprint]{tmlr}. For a camera-ready accepted version,
% change to \usepackage[accepted]{tmlr} and set \month/\year below.
% ============================================================================

\documentclass[10pt]{article}
\usepackage[preprint]{tmlr}                       % anonymized submission

% Optional math commands from https://github.com/goodfeli/dlbook_notation
\input{math_commands.tex}

\usepackage{lmodern}            % full Latin glyph set incl. em-dash
...
% Authors must not appear in the submitted version. They are hidden when
% the tmlr package is used without [accepted] or [preprint].
\author{\name Kasra Ghanavati \email kasraghanavati@icloud.com \\
        \addr Independent researcher
        \AND
        \name Ali Jabbary \email ali.jabbary@example.org \\
        \addr Independent researcher}
...
hyperparameters, not the GNN component itself.
Sixth, re-running the depth ablation on \texttt{simple\_spread} N=6
(where the algorithm does learn) we find a monotone-with-recovery
trend: $L=6$ achieves both the best mean return ($-46.48$, within a
SEM of VDN) and the tightest seed-level variance, while intermediate
depths $\{3, 4\}$ are worse than the $L=2$ baseline. With $n=5$
seeds the Holm-corrected pairwise test against $L=2$ does not reach
significance ($d=+0.40$, $p_{\text{Holm}}=1.000$), and we cannot
endorse the inverted-U prediction in its original form. Code and
full per-run logs are released at
...
(Section~\ref{sec:methods}) and a 60-run sweep across three task scales
(Section~\ref{sec:phase2c-results}), with external-validity replication
on PettingZoo's \texttt{simple\_spread} (Section~\ref{sec:phase3-results}).
We address (Q2) with a 48-run principled tuning grid
(Section~\ref{sec:phase2a-results}) that finds the configuration on which
(Q1) is testable in our env. We address (Q3) with a 75-run depth $\times$
diameter ablation (Section~\ref{sec:phase4-results}) that was designed to
test an inverted-U prediction in $L - d$ but returns a null result the
paper discusses honestly.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\linewidth]{figures/fig_phase1_pilot.png}
...
total at 10k steps and 3 seeds per cell. Figure~\ref{fig:phase2a-grid}
shows the full grid; the pattern dominates every other axis.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/fig_phase2a_tuning_grid.png}
...
$Q$-values dominate early training. The orthogonal scheme
\cite{Saxe2014Orthogonal} was originally introduced to preserve gradient
norms across layers in deep linear networks; here it has a complementary
effect: it places the hypernetwork's state-conditioned bias path at near-zero
output, where its gradient contribution to the loss is small. Default
PyTorch initialization (Kaiming-uniform) produces a state-conditioned bias
path with non-trivial magnitude that absorbs the temporal-difference error
\cite{Sutton2018RL} before the Q-network has a chance to learn. The smaller
$\eta_\text{mix}$ further slows the mixer relative to the Q-net so that the
Q-network learns first and the mixer combines a learned signal rather than
fitting the bias path.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_phase2c_forest.png}
...
\begin{figure}[t]
\centering
\begin{minipage}{0.32\textwidth}\includegraphics[width=\linewidth]{figures/fig_phase2c_curves_small.png}\end{minipage}
\hfill
\begin{minipage}{0.32\textwidth}\includegraphics[width=\linewidth]{figures/fig_phase2c_curves_medium.png}\end{minipage}
\hfill
\begin{minipage}{0.32\textwidth}\includegraphics[width=\linewidth]{figures/fig_phase2c_curves_large.png}\end{minipage}
...
The full per-(algorithm, $N$) ranking is shown in Table~\ref{tab:phase3};
trajectories are in Figure~\ref{fig:phase3-curves}.
...
\begin{figure}[t]
\centering
\begin{minipage}{0.48\textwidth}\includegraphics[width=\linewidth]{figures/fig_phase3_curves_n3.png}\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}\includegraphics[width=\linewidth]{figures/fig_phase3_curves_n6.png}\end{minipage}
...
practical lesson. Figure~\ref{fig:phase4} shows the floor result
visually: heatmap (left) and the predicted inverted-U axis (right) are
both pinned at $0.000$.

\begin{figure}[t]
\centering
\begin{minipage}{0.48\textwidth}\includegraphics[width=\linewidth]{figures/fig_phase4_heatmap.png}\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}\includegraphics[width=\linewidth]{figures/fig_phase4_inverted_u.png}\end{minipage}
...
% ============================================================================
\section{Results: GNN depth ablation on \texttt{simple\_spread}}\label{sec:phase6-results}

To make the depth-versus-performance question actually testable we
re-ran the ablation on \texttt{simple\_spread} at $N=6$ --- the env
where GNN-QMIX learns (Section~\ref{sec:phase3-results}). We vary
GNN depth $L \in \{1, 2, 3, 4, 6\}$ holding everything else fixed at
Phase 2A's locked GNN-QMIX configuration. 5 seeds per cell, 30k env
steps per run, 25 runs total.

\begin{table}[t]
\centering
\small
\caption{Phase 6: GNN-QMIX final-window evaluation return on
\texttt{simple\_spread} N=6 as a function of GNN depth $L$, mean
$\pm$ SEM over 5 seeds. Less negative is better.}
\label{tab:phase6}
\begin{tabular}{lccccc}
\toprule
$L$       & 1                   & 2 (baseline)        & 3                   & 4                   & \textbf{6} \\
\midrule
final ret & $-54.79 \pm 4.70$   & $-49.91 \pm 5.07$   & $-53.71 \pm 6.43$   & $-51.87 \pm 4.25$   & $\mathbf{-46.48 \pm 1.95}$ \\
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{figures/fig_phase6_depth_curve.png}
\caption{Phase 6: GNN-QMIX final-window evaluation return vs.\ GNN
depth $L$ on \texttt{simple\_spread} N=6. Mean $\pm$ SEM over 5 seeds
(red). Dots are individual seeds. VDN and L=2 references from Phase
3 are dashed. The deepest GNN ($L=6$) achieves both the best mean
($-46.48$) and the tightest variance (SEM $1.95$), but Holm-corrected
pairwise tests against $L=2$ do not reach significance with $n=5$.}
\label{fig:phase6}
\end{figure}

\paragraph{Trend without statistical significance.} $L=6$ achieves
both the best mean return ($-46.48$) and the tightest seed-level
variance (SEM $1.95$), comfortably better than every other depth and
within the noise of the VDN reference from Section~\ref{sec:phase3-results}
($-44.74$). The Cohen's $d$ effect size vs.\ $L=2$ is $+0.40$,
meaningful by Cohen's conventional cutoffs. But the Welch $p$ is
$0.555$ and Holm-corrected over the four contrasts vs.\ baseline it
becomes $1.000$. With $n=5$ seeds we cannot statistically distinguish
$L=6$ from $L=2$.

\paragraph{No clean inverted-U.} The intermediate depths $L \in
\{3, 4\}$ both perform worse than $L=2$, not better, contradicting
the H3 prediction of a peak at $L \approx d$. \texttt{simple\_spread}
has no natural coordination graph diameter, so $d$ is undefined;
the prediction was nonetheless that some middle depth would
out-perform extremes, and the data is monotone in the wrong direction
inside $\{1, 2, 3, 4\}$ before recovering at $L=6$. We cannot endorse
H3 in its original form on this evidence.

\paragraph{What the data does support.} $L=6$ is the empirical best
across this small sweep, with the lowest variance. If we treat $L=6$
as a calibrated configuration of GNN-QMIX and compare against the
$L=2$ default, GNN-QMIX moves from clearly worse than VDN (Phase 3:
$-49.91$ at $L=2$ vs.\ VDN's $-44.74$, Cohen's $d \approx 0.6$) to
within a SEM of VDN ($-46.48$ at $L=6$). The depth axis at least
\emph{matters} in the right direction even if our $n=5$ sample is
too small to make it statistically robust. An extension to $n=10$
on the cells $L \in \{1, 2, 6\}$ and on Phase 3's $N=6$ algorithm
ranking is underway in the public repository at the time of
submission.
...
but visible margin in Table~\ref{tab:phase1-summary}), but once both are
...
\paragraph{Value decomposition in cooperative MARL.} Cooperative MARL
with value decomposition has a well-developed lineage
\cite{Tampuu2017IQL,Sunehag2018VDN,Rashid2018QMIX,Lowe2017MADDPG}.
Independent Q-learning treats each agent as if its environment were
single-agent and ignores other agents' policy updates; value
decomposition networks (VDN) add a parameter-free sum mixer over per-agent
Q-values; QMIX adds a monotonic hypernetwork mixer conditioned on the
global state. Each step in this lineage adds capacity at the cost of
hyperparameter sensitivity, a tension we measure directly.

\paragraph{Graph neural networks.} The GNN architectures we build on
inherit from message-passing formulations
\cite{Velickovic2018GAT,Xu2019GIN}. Graph attention networks
\cite{Velickovic2018GAT} weight neighbour aggregation by learned attention;
graph isomorphism networks \cite{Xu2019GIN} were introduced to characterize
the expressive limits of message-passing under the Weisfeiler--Leman
hierarchy. Our GNN-QMIX uses a simpler mean-aggregation message-passing
block---we keep the architecture deliberately minimal so any algorithmic
advantage is attributable to the integration of graphs and value
decomposition rather than to GNN capacity.

\paragraph{GNN-MARL.} GNN augmentations of value decomposition appear in
...
the conditions under which the graph component is necessary.

\paragraph{Controlled empirical studies.} The closest methodological
precedent is the line of replication studies in deep RL that re-test
folklore claims under controlled conditions. We follow that template:
fix the per-agent Q-net architecture across algorithms so that any
between-algorithm difference is attributable to the mixer; pre-register
the locked configuration before the headline sweep so post-hoc tuning
cannot influence it; report Holm-corrected statistics over multiple
seeds; release the full per-run logs.
...
\begin{tabular}{lrrr}
\toprule
\textbf{Config} & \textbf{Return} & \textbf{Loss} & $\boldsymbol{q_\text{tot}}$ \\
\midrule
