...
% Attention arms (N=6, ego) -- all floored; inconclusive (no residual+LN):
\newcommand{\SCgatComplete}{49.65}
\newcommand{\SCdgnComplete}{50.09}
\newcommand{\SCgatTrue}{51.45}
\newcommand{\SCtrueGatTrueGap}{+14.7} % gnn_true - gat_true
\newcommand{\SCtrueGatTrueD}{4.5}
...
learning, grows with team size out to $N{=}16$, and replicates on MPE
\texttt{simple\_spread} and Level-Based Foraging---a regime where the graph
never helps. \emph{Positive endpoint.} On a constructed pair-routing task
...
the task-matched graph beats a density-matched \emph{wrong} graph and
all-to-all communication at byte-identical parameters and observations
(Cohen's $d\approx5$; $+16$ return over a random floor of ${\sim}50$ at
degree-1; Welch with Holm correction and Mann--Whitney $p<10^{-4}$). More
communication did not help; the \emph{right} communication did---so the
advantage is graph \emph{structure}, not the channel or the capacity. The
effect is significant at every team size $N{=}4$--$10$ but
\emph{non-monotone} (it does not grow as a dose-response), is robust to full
observability, and persists at degree-2 (genuine topology) though about
$\SCnbrShrink\times$ smaller. We state scope plainly: this is a
constructed-task existence/boundary result on a single environment family,
and whether learned attention can substitute for fixed structure is left
inconclusive (the attention arms lacked the stabilisation the GCN arms
carry and failed to train). The practical rule: add a coordination graph,
and route along task-relevant edges rather than all-to-all, only when agents
must act on information held by specific others they cannot observe. We
...
wrong neighbours, or at everyone, is no better than no channel. The two
...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item \textbf{The negative endpoint.} In full-information /
        ...
        variance, despite the edge-defined reward that should maximally
        reward a graph-aware model. The capacity control
        ...
        at $n{=}\PEseeds$ seeds (ruling out undertraining), grows
        monotonically with team size out to $N{=}16$, replicates on the
        standard MPE \texttt{simple\_spread} benchmark, and persists when
        the GCN is replaced by graph attention
  ...
  \item \textbf{The positive endpoint} (\S\ref{sec:results:structure}). We
        ...
        graph beats a density-matched wrong graph \emph{and} all-to-all
        communication at byte-identical parameters and observations
        ($d\approx5$ at degree-1; Welch/Holm and Mann--Whitney $p<10^{-4}$):
        more communication did not help, the \emph{right} communication did.
        The effect is significant at every $N{=}4$--$10$ (but non-monotone),
        robust to observability, and survives at degree-2 about
        $\SCnbrShrink\times$ smaller; whether learned attention substitutes
        for fixed structure is left inconclusive.
...
graph and a learned attention graph, respectively). The positive endpoint we
...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item \gnnqmix\ is markedly \emph{worse} than \qmix\ on this condition
        (mean $66.0$ vs.\ $84.0$), and the gap is not attributable to
        ...
        seed-level variance: its $95\%$ CI spans $[57.1, 74.9]$, more than
        an order of magnitude wider than \qmix's $[83.4, 84.6]$, with the
...
Mann--Whitney protocol of Section~\ref{sec:protocol}.
...
count---only the adjacency changes---the \gtrue\ vs.\ \gwrong\ and \gtrue\
vs.\ \gcomplete\ contrasts isolate \emph{structure} from channel, capacity,
and observability simultaneously.

\paragraph{Why ``a graph beats no-communication'' is not the headline.}
...
priori}. We therefore treat \gtrue\ vs.\ \mlpc\ as a sanity check and put the
load on the two controls that are \emph{not} a priori: a wrong graph of
identical density, and all-to-all communication. If structure is what
matters, both should fail despite carrying as much or more communication.
...
\SCtrueHi]$. The arm with the \emph{most} communication is no better than the
wrong graph or no communication at all; the advantage is which edges carry
messages, not how many.
...
\begin{table}[t]
  ...
  task-matched graph rises. \emph{Bottom:} the structure-isolating contrasts
  (same repaired GCN and parameter count; only the adjacency differs). $d$ is
  ...
  \gtrue\ captures only ${\sim}\SCfrac\%$ of the routing headroom
  (${\sim}\SCfloor\to{\sim}\SCoracle$): routing helps significantly, it does
  not solve the task.}
  \label{tab:structure}
  \begin{tabular}{llr}
    \toprule
    ...
    \midrule
    \multicolumn{3}{l}{\emph{Structure-isolating contrasts (only the adjacency differs):}} \\
    \gtrue$-$\gwrong    & \multicolumn{2}{l}{$\SCtwGap$,\; $d=\SCtwD$,\; $p_{\text{Holm}}\SCpHolm$,\; MWU $<10^{-4}$} \\
    \gtrue$-$\gcomplete & \multicolumn{2}{l}{$\SCtcGap$,\; $d=\SCtcD$,\; $p_{\text{Holm}}\SCpHolm$,\; MWU $<10^{-4}$} \\
    \gtrue$-$\mlpc\ {\footnotesize(a-priori sanity)} & \multicolumn{2}{l}{$\SCtmGap$,\; $d=\SCtmD$,\; $p_{\text{Holm}}\SCpHolm$,\; MWU $<10^{-4}$} \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{The decisive contrasts.} Against the wrong graph,
\gtrue\ is $\SCtwGap$ (Cohen's $d=\SCtwD$); against all-to-all, $\SCtcGap$
($d=\SCtcD$); both at $p_{\text{Holm}}\SCpHolm$ and Mann--Whitney
$p<10^{-4}$. Distributional separation is complete: the worst \gtrue\ seed
($\SCsep$) exceeds the best rival seed across all three controls
($\SCsepRivals$) over 12 seeds. The interpretation rests on these two
contrasts, both of which hold capacity, architecture, and observations fixed;
the \gtrue\ vs.\ \mlpc\ cell ($\SCtmGap$, $d=\SCtmD$) is the conceded
a-priori sanity check, not the headline. (The Mann--Whitney $p$-values sit at
...
neighbourhood set rather than a single partner---but it is about
$\SCnbrShrink\times$ smaller than at degree-1 ($+2.4$ / ${\sim}5\%$ vs.\
$+16$ / ${\sim}33\%$). A fixed mean-aggregating GCN exploits a
neighbourhood-set (centroid) task far less efficiently than single-partner
routing.

\subsection{What we do \emph{not} claim: attention vs.\ structure}
\label{sec:structure:attention}

A natural question is whether learned attention over the \emph{complete}
graph could rediscover the right edges and thus substitute for fixed
structure. We tested single-head (GAT) and multi-head (DGN/G2ANet-style)
attention over all-to-all; as run, both stayed at the floor
($\SCgatComplete$, $\SCdgnComplete$). \textbf{We do not conclude ``attention
cannot substitute for structure,'' because the attention arms are not cleanly
identified:} they lacked the residual$+$LayerNorm stabilisation the GCN arms
carry, and GAT \emph{given the correct graph} (\texttt{gat\_true}) also
floored ($\SCgatTrue$, vs.\ \gtrue\ at $66.1$: a $\SCtrueGatTrueGap$ gap,
$d=\SCtrueGatTrueD$). The attention arms thus failed to \emph{train}, not
demonstrably to \emph{find} structure. The ``structure, not channel''
conclusion rests entirely on the clean \gtrue\ vs.\ \gcomplete\ contrast
(both repaired GCN; $\SCtcGap$, $d=\SCtcD$). A matched-stabilisation
attention re-run (adding residual$+$LayerNorm to GAT/DGN) is the cheapest
open follow-up; until then we report attention-vs-structure as inconclusive.
...
negative was wrong. Two limits are explicit. (i) It is a constructed-task
existence/boundary result on a single environment family (the $3{\times}3$
\coordgrid, $N\le10$); the negative half replicated out of harness on
MPE/LBF, but the positive half has not yet, so we do not generalise beyond
``task-structured routing under partial information.'' (ii) Whether learned
attention can recover the effect is open (\S\ref{sec:structure:attention}).
For reproducibility, the $N{=}6$ headline is \emph{bit-identical} across two
...
significantly below \qmix\ at three ($d=-2.24$, Welch $p=0.014$)---and a
preliminary single-seed StarCraft~II (\texttt{3m}) check is neutral ($0.92$
vs.\ $1.00$ win rate; Appendix~\ref{app:standard}). In this regime the
measured effect ranges from neutral to harmful and is never positive.
...
conditional---reading of graph-based value factorisation than the prevailing
positive framing \citep{jiang_2020_dgn,liu_2020_g2anet,naderializadeh_2020_graphmix}
...
the rule sharp: at our positive endpoint, all-to-all communication and a
density-matched wrong graph were both no better than no communication at all
(Section~\ref{sec:structure:headline}). Pointing the channel at the wrong
neighbours wastes it; pointing it at everyone does not rescue it. Where the
...
controls show both conditions are necessary, not just sufficient: at the
positive endpoint, removing alignment (the wrong graph) or flooding the
channel (all-to-all) each collapsed the advantage to zero
(Section~\ref{sec:structure:headline}). The same aggregation that is a
...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item \textbf{Positive endpoint is a single-env existence result.} The
        positive boundary (Section~\ref{sec:results:structure}) is a
        constructed-task existence/boundary result on one environment family
        (the $3{\times}3$ \coordgrid, $N\le10$). Unlike the negative half, it
        has not yet been replicated out of harness, so we do not generalise it
        beyond ``task-structured routing under partial information.''
  \item \textbf{Attention vs.\ structure is inconclusive.} The attention arms
        at the positive endpoint (GAT/DGN over the complete graph) lacked the
        residual$+$LayerNorm stabilisation the GCN arms carry and failed to
        train---GAT \emph{given the correct graph} also floored
        (Section~\ref{sec:structure:attention}). We therefore cannot say
        whether learned attention can substitute for fixed structure; a
        matched-stabilisation re-run is the cleanest follow-up.
...
survives $5\times$ training, replicates on MPE \texttt{simple\_spread} and
Level-Based Foraging, persists under graph attention, grows monotonically to
$N{=}16$, and inflates across-seed variance ${\sim}9\times$; the pre-registered
...
beats a density-matched wrong graph \emph{and} all-to-all communication at
byte-identical parameters and observations ($d\approx5$ at degree-1;
Welch/Holm and Mann--Whitney $p<10^{-4}$)---more communication did not help,
the \emph{right} communication did, so the advantage is structure, not the
channel. The effect is significant at every $N{=}4$--$10$ (non-monotone),
robust to observability, and survives at degree-2 about $\SCnbrShrink\times$
smaller. The practical rule that unifies the two: route along task-relevant
edges, not all-to-all, only when agents must act on information held by
specific others they cannot observe. Two follow-ups would sharpen the
positive endpoint: replicating it out of harness on a standard
partially-observable benchmark, and a matched-stabilisation attention re-run
(adding residual$+$LayerNorm to GAT/DGN) to settle whether learned attention
can substitute for fixed structure. We release the harness and the
\mlpqmix\ control to make such measurements cheaper.
...
anonymized supplementary material. The exact commands used to produce every
...
Welch $p=0.014$, Mann--Whitney $p=0.032$) and below the parameter-matched
control ($d=-1.11$; non-significant only because the control is bimodal
across seeds), while \mlpqmix\ matches \qmix\ (capacity neutral). The graph
...
The map is near-saturated and this is a single seed, so we read it only as
directionally consistent with the LBF and \coordgrid\ results---the graph does
not outperform the matched control---and leave a full multi-seed SMAC study to
...
control. With the harmful effect on \coordgrid\ and MPE
(Section~\ref{sec:results:capacity}), the graph's measured effect ranges from
...
VDN & 10 & 66.02 & [62.57, 69.48] & 1528 \\
QMIX & 10 & 84.02 & [83.41, 84.62] & 1431 \\
GNN-QMIX & 10 & 66.02 & [57.11, 74.92] & 1552 \\
    \bottomrule
  \end{tabular}
\end{table}
