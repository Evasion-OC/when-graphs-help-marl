...
$U$, the latter robust to the variance blow-up reported below. Full
per-cell numbers are in Table~\ref{tab:phase8} (Appendix~\ref{app:figures});
the findings below state the decisive values inline.
...
single number per algorithm. Our results suggest such comparisons are
heavily condition-dependent --- which algorithm wins is a function of the
graph regime, not the algorithm --- and that future cooperative-MARL
benchmarks should expose the coordination graph as a controllable axis, in
the spirit of \coordgrid.
...
point: ``does graph structure help in MARL?'' demands controlled,
capacity-matched measurement, and on the task most favourable to a
graph-aware model the answer, at the scales we can test, is ``no, and it
actively hurts.'' We release the harness and the \mlpqmix\ control to make
such measurements cheaper for future architectures and tasks.
