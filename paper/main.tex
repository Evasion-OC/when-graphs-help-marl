...
communication did not help; the \emph{right} communication did---and the
advantage is graph \emph{structure}, not the channel, the capacity, or the
aggregator's expressiveness: even a stabilisation-matched \emph{learned}
attention aggregator floors over all-to-all while matching the GCN given the
right graph, so it needs the structure too. The effect is significant at every
...
(genuine topology) though about $\SCnbrShrink\times$ smaller. It replicates
...
still future work. The practical rule: add a coordination graph,
...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item \textbf{The positive endpoint} (\S\ref{sec:results:structure}). We
        ...
        The advantage is the structure, not the channel, the capacity, or the
        aggregator's expressiveness: even a stabilisation-matched learned
        attention aggregator floors over all-to-all yet matches the GCN given
        the right graph. The effect is significant at every $N{=}4$--$10$ (but
        non-monotone), robust to observability, survives at degree-2 about
        $\SCnbrShrink\times$ smaller, and replicates out of harness in a
        non-spatial token-matching game (not a gridworld artefact).
...
arms lacking the GCN's stabilisation, is removed. Two facts settle the
question. \emph{(i) The attention arms are not under-trained:} given the
\emph{right} graph, learned attention is statistically indistinguishable from
the GCN (\texttt{gat\_true} $\SCgatTrue$ vs.\ \gtrue\ at \SCtrueN; the gap
$\SCgatTrueGap$ nominally favours attention but is not significant, Welch
$p=\SCgatTrueP$). \emph{(ii) Over
all-to-all, learned attention still floors:} \gtrue\ beats
\texttt{gat\_complete} ($\SCgatComplete$) and \texttt{dgn\_complete}
($\SCdgnComplete$) by $\SCtrueGatCompGap$ and $\SCtrueDgnCompGap$
respectively (Cohen's $d{\approx}\SCtrueAttCompD$, $p<10^{-4}$). So even an
expressive, well-trained attention aggregator does \emph{not} recover the
result from the wrong structure---it needs the right graph too. The advantage
is the structure, not the channel, the capacity, \emph{or} the aggregator's
expressiveness.
...
channel, the capacity, \emph{or} the aggregator's expressiveness: a
stabilisation-matched learned attention aggregator floors over all-to-all yet
matches the GCN given the right graph. The effect is significant at every
$N{=}4$--$10$ (non-monotone), robust to observability, survives at degree-2
about $\SCnbrShrink\times$ smaller, and replicates out of harness in a
