...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  ...
  \item \textbf{GNN family.} We test three relational mechanisms --- a fixed
        ...
        attention (Appendix~\ref{app:gat}); attention narrows but does not
        close the gap. Learned message passing (\algoname{GIN}) and
        edge-feature GNNs remain open.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/phase10_gat.pdf}
  \caption{Single-head (\gatqmix) and multi-head (\dgnqmix) graph attention
  both sit above \gnnqmix\ (the fixed GCN) --- more expressive attention
  helps \emph{within} graph methods --- but still below the
  parameter-matched no-graph control \mlpqmix\ at both team sizes on
  CoordGrid: the graph in its strongest form remains a net liability.}
  \label{fig:gat}
\end{figure}
...
\begin{table}[h!]
  ...
  (\dgnqmix, the DGN/G2ANet operator) attention land above \gnnqmix\ ---
  more expressive attention helps \emph{within} graph methods --- but below
