...
in small-team CTDE settings. Crucially, the identified graph penalty is
not an artefact of our instrument: on the standard MPE
\texttt{simple\_spread} benchmark it reappears at the larger team size
(Cohen's $d=\PMgraphDNsix$), where --- as on \coordgrid\ --- only a
parameter-matched control separates the graph penalty from a partially
cancelling capacity gain that leaves the naive comparison looking null.
The penalty is moreover not specific to the GCN --- it persists when the
graph uses learned attention (the mechanism of the methods we critique) ---
and it \emph{grows} monotonically with team size, out to $N{=}16$.
...
\begin{table}[t]
  \centering
  \small
  \caption{Phase 3 external validity on MPE \texttt{simple\_spread} (mean
  final-window return over \PEseeds\ seeds; less negative is better). The
  decomposition is the point: at $N{=}6$ the \emph{capacity} effect
...
on this task (capacity effect $\PMcapGapNsix$, $d=\PMcapDNsix$;
Mann--Whitney $p=\PMcapMwuNsix$, marginal after Holm at $p{=}0.07$), and
that gain nearly cancels the graph penalty
($\PMcapGapNsix\!+\!\PMgraphGapNsix\approx\PMnetNsix$). Only the
parameter-matched \mlpqmix\ separates the two, exposing a large graph
liability that the headline comparison masks --- on a standard benchmark,
precisely the confound this paper warns about.
...
scale.} Two robustness checks close the most natural escape routes
(Appendices~\ref{app:gat}--\ref{app:scaling}). \emph{(i) GNN family.} The
obvious objection is that the learned attention used by the methods we
critique \citep{jiang_2020_dgn,liu_2020_g2anet} would rescue the graph. It
does not: replacing the GCN with single-head graph attention (\gatqmix,
carrying marginally \emph{more} parameters than \mlpqmix) still loses to
the no-graph control by $\PGgatMlpNfour$ ($d=\PGgatMlpDNfour$) at $N=4$ and
$\PGgatMlpNeight$ ($d=\PGgatMlpDNeight$) at $N=8$ (Holm and Mann--Whitney
significant). Attention \emph{does} beat the fixed GCN ($+\PGgatGnnNeight$
at $N=8$) --- the graph in its strongest form --- yet stays a net
liability, and the same direction holds on MPE. \emph{(ii) Scale.}
Extending to $N\in\{12,16\}$, the graph effect grows \emph{monotonically}:
$\PSgapNfour$, $\PSgapNeight$, $\PSgapNtwelve$, $\PSgapNsixteen$ at
$N=4,8,12,16$; at $N=16$ \gnnqmix\ reaches only
...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  \item \textbf{Scale.} Our largest team is $N{=}16$ on a
        tabular-flavoured gridworld; behaviour on $N{\gg}16$ and on
        visually complex tasks (SMAC-scale) is out of scope. We note the
        graph penalty \emph{grows} monotonically out to $N{=}16$
        (Finding 5, Appendix~\ref{app:scaling}), so the negative result is
        if anything strengthened, not threatened, by scale within the range
        we can afford. (Step budget is likewise not a concern: the decisive
        cells run at \PEsteps\ steps, $5\times$ the main sweep, unchanged.)
  \item \textbf{Coordination-graph design.} \coordgrid\ ties reward to
        graph edges directly --- the cleanest test of graph-aware models,
        but also the most favourable to them. We mitigate this with the
        MPE \texttt{simple\_spread} replication
        (Section~\ref{sec:results:mpe}), where the graph is an
        unprivileged $k$-NN proximity graph the task never references and
        the penalty still reappears at $N{=}6$. We have thus tested two
        graph \emph{families}, not the full space; tasks whose true
        coordination structure differs from any observable graph may still
        behave differently.
  \item \textbf{GNN family.} We test two families --- a fixed Kipf--Welling
        GCN and single-head graph attention (\gatqmix, Finding 5,
        Appendix~\ref{app:gat}). Attention narrows the gap but does not
        close it, so the penalty is not GCN-specific. Whether multi-head
        attention, learned message passing (e.g.\ \algoname{GIN}), or
        edge-feature GNNs behave differently remains open.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.72\textwidth]{figures/phase9_mpe_capacity.pdf}
  \caption{Capacity control on MPE \texttt{simple\_spread} (\PEseeds\
  seeds, 95\% Student-$t$ CIs; less negative is better). At $N{=}6$,
  \mlpqmix\ (matched parameters, no graph) sits above \qmix\ while
  \gnnqmix\ falls below both --- the positive capacity effect and the
  negative graph effect that the main-text decomposition
  (Table~\ref{tab:mpe}) separates.}
  \label{fig:mpe}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.86\textwidth]{figures/phase10_gat.pdf}
  \caption{GAT-QMIX on CoordGrid \texttt{ring}. \gatqmix\ (attention) sits
  above \gnnqmix\ (the fixed GCN) but still well below the parameter-matched
  no-graph control \mlpqmix\ at both team sizes --- the graph's strongest
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/phase11_scaling.pdf}
  \caption{Left: final-window return vs.\ team size --- the graph-free
  curves (\qmix, \mlpqmix) climb together while \gnnqmix\ flatlines. Right:
  the graph penalty ($\mlpqmix-\gnnqmix$) grows monotonically with $N$.}
  \label{fig:scaling}
\end{figure}
...
\begin{figure}[h!]
  ...
  \caption{Phase 8 capacity control (the bar-chart view of
  Table~\ref{tab:phase8}). \mlpqmix\ (same parameter count as \gnnqmix, no
  graph) tracks \qmix, while \gnnqmix\ falls far below both at both team
  sizes --- so the penalty is the graph aggregation, not the added
  capacity. Bars are means over \PEseeds\ seeds; whiskers are 95\%
  Student-$t$ CIs.}
  \label{fig:phase8}
\end{figure}
