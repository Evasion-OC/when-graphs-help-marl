...
% To submit, swap \documentclass{article} for the TMLR style and set
% \jmlrproceedings{...} per the journal's instructions; everything else here
% is venue-agnostic standard LaTeX.
% =========================================================================
...
graph-diameter grid (H3). Three findings emerge. First, \gnnqmix\ is
worse than the structurally simpler \qmix\ in \emph{every} condition
we tested --- a 16-cell sweep across $N \in \{4, 8\}$, two graph
regimes, and four algorithms (one-sided $p \in [0.98, 1.00]$ that
\gnnqmix\ improves on \qmix), with gaps ranging from $-60$ to $-147$
return units. Second, H2's directional prediction (structure helps
\gnnqmix\ more than matched-density random graphs) is supported at
$N=4$ ($p=0.007$) but reverses at $N=8$. Third, the receptive-field
alignment heuristic ($L \approx d$) is falsified: the data reveal an
...
\begin{enumerate}[leftmargin=*,itemsep=0pt,topsep=2pt]
  \item \textbf{H1 is conclusively falsified.} In every $(N, \text{graph})$
  ...
  \item \textbf{\gnnqmix\ fails to learn at all in two of the four
        ...
        re-sampled graph structure between episodes appears to be a
        substantially harder regime for the GCN encoder than the fixed
        ring; the over-depth-cliff hypothesis advanced in
        Section~\ref{sec:results:ablation} is one likely explanation
        but the present data underdetermine the mechanism.
\end{enumerate}
...
(mixed). Owing to the compute envelope, we report a restricted replication
rather than a full sweep. [PLACEHOLDER for either: results of restricted
runs, OR a statement that this is deferred to future work, with the adapter
...
\paragraph{Where graph structure helps in our experiments: nowhere.}
Across the 16-cell Phase-2 sweep (Sections~\ref{sec:results:scale}) and
the depth ablation (Section~\ref{sec:results:ablation}),
\gnnqmix\ never improves on the structurally identical \qmix\ baseline.
In every $(N, \text{graph})$ condition \qmix\ is significantly better
(one-sided $p \in [0.98, 1.00]$); the only positive directional result
is H2 at $N=4$, where the \gnnqmix-\qmix\ gap is \emph{less negative}
on the ring than on Erd\H{o}s--R\'enyi --- a difference between two
losses, not a positive case for the graph mixer. This is a substantially
more constrained reading of graph-based MARL than the prevailing
positive framing in the literature
\citep{jiang_2020_dgn,liu_2020_g2anet,naderializadeh_2020_graphmix}
allows, and is consistent with the broader evaluation-rigour concerns
of \citet{henderson_2018_drlmatters} and \citet{agarwal_2021_precipice}.
Two caveats are central: (i) we test a single GNN family (vanilla GCN,
no attention or learned edge features) and (ii) the compute envelope
limits per-condition seeds to three, so the absence of a positive case
in our data does not preclude one in larger-scale or differently-
configured studies. What our data \emph{do} establish is that the
positive claim cannot be assumed to hold by default.
