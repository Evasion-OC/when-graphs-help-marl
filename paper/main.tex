...
Cohen's $d \geq 3.5$). Scale dominates algorithm choice.
Fourth, the locked Phase~2A configurations do not transfer to a higher
state-dimension benchmark
(\todoFromPhase{Phase 3}{numbers TBD; CPU smoke indicates QMIX family
diverges on simple\_spread while VDN and IQL remain stable}), and we
characterize when the GNN advantage---if any---reappears as graph
diameter changes
(\todoFromPhase{Phase 4}{numbers TBD; pre-flight smoke clean}).
Code and full per-run logs are released at
...
\todoFromPhase{Phase 4}{whether the GNN--QMIX gap reopens when the
underlying coordination graph has non-trivial diameter and the GNN
depth is matched to it---that ablation is the cleanest test of the
``graphs help when graphs help'' hypothesis.}
...
The remaining test of when graph structure helps is the depth--diameter
ablation \todoFromPhase{Phase 4}{being run}; the cross-env transfer
question is partially answered by the simple\_spread results
\todoFromPhase{Phase 3}{being run}. The claim that ``GNN helps in
MARL'' should be qualified by the conditions of the test: the field's
published default hyperparameters do not transfer to small-state
problems, and the GNN advantage outside that regime---if any---is
smaller than informal claims suggest.
