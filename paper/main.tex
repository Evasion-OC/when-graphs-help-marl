...
\PEseeds\ seeds), \emph{destabilises} learning, and \emph{grows} with team size out to
$N{=}16$. The penalty is not an artefact of our instrument: it replicates
on the standard MPE \texttt{simple\_spread} benchmark and persists under
learned graph attention, while on Level-Based Foraging the penalty re-emerges with team size
(significantly harmful at 3 agents) and a preliminary SMAC check is \emph{neutral}. Across every setting it
never \emph{helps}---its effect ranges from neutral to harmful. Second, the depth--diameter alignment heuristic (H3)
...
\paragraph{Does graph structure ever help?} Across every condition we
tested in Sections~\ref{sec:results:scale}--\ref{sec:results:capacity},
the \gnnqmix\ encoder does \emph{not} outperform the structurally
identical \qmix\ baseline. The capacity control
(Section~\ref{sec:results:capacity}) shows this is not because the extra
parameters are wasted: \mlpqmix, carrying the same parameters without the
graph, matches \qmix, which isolates the effect on the graph aggregation
itself. Where that aggregation's receptive field is misaligned with the
task's coordination structure, on \coordgrid\ and on the MPE benchmark, it
is actively \emph{harmful}: \gnnqmix\ falls well below both controls, and
the penalty survives graph attention and grows out to $N{=}16$. On standard
cooperative benchmarks usually credited to graph methods the same picture
holds: on Level-Based Foraging the penalty tracks team size---neutral at two
agents but significantly below \qmix\ at three ($d=-2.24$, Welch $p=0.014$)---and
a preliminary single-seed StarCraft~II (\texttt{3m}) check is neutral
($0.92$ vs.\ $1.00$ win rate; Appendix~\ref{app:standard}). The consistent reading is therefore not that
the graph always hurts but that \emph{it never helps}: its measured effect
ranges from neutral to harmful and is never positive.
...
small-to-moderate cooperative tasks, not the full published systems at SMAC scale.
...
control, and the answer, at the scales we test, is \emph{no}: the graph is
harmful where the coordination structure is non-trivial---including
Level-Based Foraging at three agents---and neutral where it is trivial or
saturated, never beneficial. We release the
