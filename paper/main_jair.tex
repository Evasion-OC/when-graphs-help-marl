...
\begin{abstract}
Graph neural network (GNN) value-factorisation methods are widely claimed to
beat graph-free baselines in cooperative multi-agent reinforcement learning
(MARL) by exploiting relational structure. Through a controlled study on
\coordgrid, a cooperative gridworld whose coordination graph is the experimental
variable, we show the claim has a sharp boundary and locate both sides.

\emph{Positive endpoint.} On a routing task where each agent must reach a
partner's \emph{private} goal, held by one specific unobserved agent, a graph
whose edges match the task's coordination structure decisively beats all-to-all
communication, a density-matched wrong graph, and a no-graph control---at
byte-identical parameters and observations (Cohen's $d$ of about five; complete
seed-level separation over twelve seeds, Holm-corrected). What helps is the
\emph{structure}, not the channel or the capacity: all-to-all carries the
partner's signal but cannot select it, and even learned attention recovers it
only on the right graph. The effect holds across team size and observability,
survives degree-two topology, and replicates out of harness in a token-matching
game.

\emph{Negative endpoint.} When coordination is incidental or
convention-reducible, the GNN \emph{underperforms} the simpler \qmix; a
parameter-matched no-graph control (\mlpqmix) pins the cause to graph
aggregation, not capacity. The penalty persists under five times the budget,
grows with team size, and recurs on standard MPE and Level-Based Foraging
benchmarks.

These are controlled, constructed tasks; a standard third-party benchmark with
private goals remains future work. The rule: add a coordination graph and route
along task-relevant edges---not all-to-all---only when agents must act on
information held by specific unobserved others \emph{and} the topology is known
or estimable. We release the full code and configurations
