...
% --- Title block. ---------------------------------------------------------
% Single \title declared in the preamble so hyperref captures the full title for
% the PDF metadata (pdftitle). The short form feeds running heads; the long form
% feeds the title page. No line break (\\) in the title, so hyperref emits no
% "token not allowed in PDF string" warning; acmart wraps the long title itself.
...
\renewcommand{\shortauthors}{Jabbary \& Ghanavati}

\begin{abstract}
Graph neural network (GNN) value-factorisation methods are widely held to
improve cooperative multi-agent reinforcement learning (MARL) by exploiting
relational structure, yet when they actually help has not been isolated. We
settle the question with a controlled study on \coordgrid, a cooperative
gridworld whose coordination graph is the experimental variable, with the
encoder, mixer, and optimiser held fixed across algorithms. The benefit is real
but conditional, and we identify exactly when it appears. When an agent must act
on information held by one specific, unobserved other---a private-goal routing
task---a graph whose edges match the task's coordination structure decisively
outperforms all-to-all communication, a density-matched wrong graph, and a
parameter-matched no-graph control, at byte-identical parameters and observations
(Cohen's $d$ of about five; complete seed-level separation over twelve seeds,
Holm-corrected). The advantage is the graph \emph{structure}, not the
communication channel or the capacity: all-to-all carries the partner's signal
but cannot select it, and even learned attention recovers it only on the right
graph. The effect holds across team size and observability and replicates out of
harness in a non-spatial token-matching game. When coordination is instead
incidental or convention-reducible, the same GNN \emph{underperforms} a
structurally simpler baseline, and a matched control attributes the loss to the
aggregation itself, not the added capacity---a penalty that persists with more
training, grows with team size, and recurs on standard MPE and Level-Based
Foraging tasks. We give a precise rule---route along task-relevant edges, not
all-to-all, only when agents must act on information held by specific others they
cannot observe \emph{and} the coordination topology is known or reliably
estimable---and state scope plainly: these are controlled, constructed tasks,
with replication on a standard third-party benchmark left to future work. We
release the full code and configurations (Appendix~\ref{app:repro}).
\end{abstract}
...
\begin{figure}[t]
  ...
  \Description{Two-panel diagram. Left (A): a 4x4 toroidal grid with four
  colored agent icons connected by a ring coordination graph; reward edges
  light up when two linked agents share a cell. Right (B): a block diagram
  showing all algorithms sharing the same observation encoder and QMIX
  mixer, with QMIX bypassing the middle block, GNN-QMIX inserting an
  L-layer GCN, and MLP-QMIX inserting a parameter-matched MLP without
  graph aggregation.}
  \label{fig:overview}
\end{figure}
...
\begin{figure}[t]
  ...
  \Description{Line plot with two panels for N=3 and N=6 on MPE simple\_spread
  (y-axis: final-window return, less negative is better). At N=3 the three
  algorithm curves nearly overlap. At N=6 MLP-QMIX sits highest, QMIX below it,
  and GNN-QMIX lowest, illustrating that the capacity control exposes a graph
  penalty invisible in the naive GNN-QMIX vs. QMIX comparison.}
  \label{fig:mpe}
\end{figure}
...
\begin{figure}[t]
  ...
  Student-$t$ CIs.}
  \Description{Bar chart comparing final-window return for QMIX, MLP-QMIX,
  and GNN-QMIX on the CoordGrid ring at N=4 and N=8. QMIX and MLP-QMIX bars
  are nearly equal at both team sizes; the GNN-QMIX bar is substantially
  shorter, especially at N=8, showing the graph penalty grows with team size.}
  \label{fig:phase8}
\end{figure}
...
\begin{figure}[t]
  ...
  \Description{Two side-by-side learning-curve panels (N=4 left, N=8 right;
  x-axis: environment steps up to 150k; y-axis: episode return). In both
  panels the QMIX and MLP-QMIX lines rise together to a high plateau with
  tight confidence bands, while the GNN-QMIX line is lower and has a much
  wider shaded band reflecting high seed-to-seed variance.}
...
\begin{figure}[t]
  ...
  \Description{Grouped bar chart at N=4 and N=8 on CoordGrid ring showing
  five algorithms: QMIX and MLP-QMIX bars are tallest and nearly equal;
  GNN-QMIX is lowest; GAT-QMIX and DGN-QMIX bars fall between GNN-QMIX and
  MLP-QMIX, showing that more expressive attention improves within the graph
  family but remains below the no-graph control at both team sizes.}
  \label{fig:gat}
\end{figure}
...
\begin{figure}[t]
  ...
  \Description{Two-panel figure. Left: line plot with x-axis team size N from
  4 to 16 and y-axis final return; QMIX and MLP-QMIX lines rise together while
  GNN-QMIX is nearly flat, widening the gap monotonically. Right: bar chart of
  the graph penalty (MLP-QMIX minus GNN-QMIX) at each N, growing from about 10
  at N=4 to over 260 at N=16, illustrating the monotone penalty scaling.}
  \label{fig:scaling}
\end{figure}
...
\begin{figure}[t]
  ...
  observations, 30k steps). Only the \emph{task-matched} graph rises above the
  random floor (${\sim}50$): the no-graph control (\mlpc), a density-matched
  wrong graph, and the all-to-all graph (the \emph{most} communication) all sit
  at the floor, while the task-matched graph reaches $66$--$71$ depending on the
  ...
  GCN---more communication does not help, the right communication does.}
  \Description{Grouped bar chart with four x-axis groups: no graph, wrong graph,
  all-to-all graph, and task-matched graph. The first three groups sit on a
  dashed horizontal floor line near 50. The task-matched group has three bars
  for the GCN, GAT, and DGN aggregators rising to about 66, 67, and 71, well
  above the floor, annotated with an effect size of d about 5.}
  \label{fig:structure}
\end{figure}
...
\begin{figure}[t]
  ...
  replication.} On \tokenmatch, a non-spatial referential game with no grid or
  movement, the task-matched graph ($57.6$) separates completely from all-to-all
  ($24.5$), the wrong graph ($12.0$), and no graph ($11.9$)---so the effect is
  not a gridworld artefact. All bars/points are 95\% CIs over 12 seeds.}
  \Description{Two panels. Left: a line for the task-matched graph rising with
  team size N from 4 to 10, staying above a shaded floor band, with gap labels
  plus 11.6, 15.7, 16.1, and 11.2 showing a non-monotone advantage that peaks at
  N equals 8. Right: a bar chart on the TokenMatch game where the task-matched
  bar reaches about 58 while the all-to-all, wrong-graph, and no-graph bars sit
  at about 24, 12, and 12.}
...
\begin{figure}[t]
  ...
  routing.}
  \Description{Grouped bar chart with three conditions: pair-routing ego
  observations, pair-routing full observations, and degree-2 topology. In each,
  a gray floor bar near 50 sits beside an orange task-matched bar. The orange
  bar exceeds the floor by about 16 for ego, 23 for full observability, and only
  about 2 for degree-2.}
...
Figure~\ref{fig:scaling-replication}, right),
...
\begin{figure}[!ht]
  ...
  \Description{Line plot with x-axis environment steps (0 to 50k) and y-axis
  episode return. Four colored curves with shaded standard-deviation bands:
  QMIX rises fastest to near 84; VDN and GNN-QMIX converge near 66 with wider
  bands; IQL plateaus near 35. All four algorithms train to completion.}
...
\begin{figure}[!ht]
  ...
  \Description{Line plot with x-axis GCN depth L from 1 to 4 and y-axis
  final-window return for three curves, one per graph diameter d=1, 2, 3.
  All three curves are relatively high at L=1 and L=2, then drop sharply at
  L=3 and L=4, forming a consistent over-depth cliff regardless of diameter.}
